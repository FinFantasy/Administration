                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-default btn-xs btn-info"
                                        onclick="location.href='/teacher/gradeCourse?id=${item.courseid}'">成绩</button>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
                ...
                <div class="panel-footer">
                    <c:if test="${pagingVO != null}">
                        <nav style="text-align: center">
                            <ul class="pagination">
                                <li><a href="/teacher/showCourse?page=${pagingVO.upPageNo}">&laquo;</a></li>
                                <c:choose>
                                    <c:when test="${pagingVO.currentPageNo<=3 && pagingVO.totalPage>=5}">
                                        <c:forEach begin="1" end="5" var="i">
                                            <c:choose>
                                                <c:when test="${pagingVO.currentPageNo == i}">
                                                    <li class="disabled"><a href="/teacher/showCourse?page=${i}">${i}</a></li>
                                                </c:when>
                                                <c:otherwise>
                                                    <li><a href="/teacher/showCourse?page=${i}">${i}</a></li>
                                                </c:otherwise>
                                            </c:choose>
                                        </c:forEach>
                                    </c:when>
                                ...
                                <li><a href="/teacher/showCourse?page=${pagingVO.nextPageNo}">&raquo;</a></li>
                                <li><a href="/teacher/showCourse?page=${pagingVO.totalPage}">最后一页</a></li>
                            </ul>
                        </nav>
                    </c:if>
                </div>
